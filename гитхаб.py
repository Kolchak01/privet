...
class GitaddMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig("GH_TOKEN", "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", lambda m: self.strings("cfg_token", m),
            "GH_USERNAME", "Kolchak01", lambda m: self.strings("cfg_gh_user", m),
            "GH_REPO", "privet", lambda m: self.strings("cfg_gh_repo", m))
    ...
    async def gitaddcmd(self, message): 
        ...
        try:
            ...
            if int(r.status_code) == 201:
                uploaded_to = f'"https://raw.githubusercontent/{USERNAME}/{REPO}'
                uploaded_to_raw = uploaded_to + f'/master/{fname}"'
